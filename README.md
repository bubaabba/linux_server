## Credit [*stueken*](https://github.com/stueken) who wrote a really helpful README in his [repository](https://github.com/stueken/FSND-P5_Linux-Server-Configuration).
...
IP address: 52.56.172.157

Accessible SSH port: 2200.

Application URL: http://ec2-52-56-172-157.eu-west-2.compute.amazonaws.com/].

## Step by step walkthrough

### 1 - Create a new user named *grader* and grant this user sudo permissions.

1. Log into the remote VM as *root* user through ssh: `$ ssh root@http://52.56.172.157/`.
...
1. Generate an encryption key **on your local machine** with: `$ ssh-keygen -f ~/.ssh/keys`.
2. Log into the remote VM as *root* user through ssh and create the following file: `$ touch /home/grader/.ssh/authorized_keys`.
3. Copy the content of the *keys.pub* file from your local machine to the */home/grader/.ssh/authorized_keys* file you just created on the remote VM. Then change some permissions:
	1. `$ sudo chmod 700 /home/grader/.ssh`.
	2. `$ sudo chmod 644 /home/grader/.ssh/authorized_keys`.
	3. Finally change the owner from *root* to *grader*: `$ sudo chown -R grader:grader /home/grader/.ssh`.
4. Now you are able to log into the remote VM through ssh with the following command: `$ ssh -i ~/.ssh/keys grader@52.56.172.157`.
...
2. `$ sudo service ssh restart`.
3. Now you are able to log into the remote VM through ssh with the following command: `$ ssh -i ~/.ssh/keys -p 2200 grader@52.56.172.157`.
...
3. Move inside that newly created folder: `$ cd /catalog` and clone the catalog repository from Github: `$ git clone https://github.com/bubaabba/catalog.git `.
...
```
<VirtualHost *:80>
    ServerName 52.56.172.157
    ServerAlias ec2-52-56-172-157.us-west-2.compute.amazonaws.com
    ServerAdmin admin@52.56.172.157
